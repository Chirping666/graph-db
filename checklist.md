...
Confirm the workspace compiles and all tests pass before making any changes.
...
### 0.2 — Workspace Cargo.toml: switch to explicit resolver

Add `resolver = "2"` to the workspace `[workspace]` table if not already present.
This is required for the new three-crate layout.
...
- **Do not delete the files yet if they are needed for Phase 2** — copy them to a
  temporary staging location (e.g., `/tmp` or a `_staging/` dir) before removing,
  so they can be moved into `phonograph_db` later. Alternatively, use `git` to
  recover them from the pre-rename commit.
...
All pass. The workspace as a whole will NOT compile yet — `graph_db` has broken
imports. This is expected and will be resolved in Phase 2.
...
**Verify:** Both commands pass. This is the critical gate — the database engine
...
All pass. The workspace still won't fully compile because `graph_db` (the old
top-level crate) has had its `src/storage/`, `src/db/`, and `src/backend_std/`
gutted. Phase 3 addresses this.
...
### 4.1 — Decide: facade or delete

**Option A (facade):** Replace `graph_db`'s `src/lib.rs` with a single re-export:
```rust
pub use phonograph_std::*;
```
Update `graph_db`'s `Cargo.toml` to depend only on `phonograph_std`. Remove
`crc32fast`, `xxhash-rust`, `libc` — those now belong to their respective crates.

**Option B (delete):** Remove `graph_db` from the workspace entirely. Move
`tests/`, `examples/`, `fuzz/`, `README.md`, `CHANGELOG.md`, and licenses to the
workspace root (or into `phonograph_std`). Update the workspace `Cargo.toml`.
...
### 5.5 — Regression test count

Compare `cargo test --workspace` output against the Phase 0 baseline.
All 311+ (or 473+, depending on baseline) tests must still pass.
...
All 13 checks from `030-workspace-redesign.md` §17 pass. The workspace is a clean
three-crate layout. Commit with:
