...
use phonograph::inference::InferenceMode;
use phonograph::types::{NodeId, EdgeId, Value};
use phonograph::{NotFoundError, SchemaError};
...
fn e2e_cross_backend_equivalence() {
    ...
    fn populate<B: phonograph_db::backend::StorageBackend>(db: &phonograph_db::db::Database<B>) -> (Vec<NodeId>, Vec<EdgeId>) {
        let mut wtx = db.write_txn().unwrap();
        let nt = wtx
