...
use phonograph::{InferenceError, NotFoundError};
...
/// wtx.commit().unwrap();
/// ```
pub struct WriteTransaction<'db, B: crate::backend::StorageBackend> {
...
impl<'db, B: crate::backend::StorageBackend> WriteTransaction<'db, B> {
    ...
    fn base_reader(&self) -> BaseSnapshotReader<'_, 'db, B> {
    ...
    pub fn all_edges(&self) -> Result<Vec<Edge>, Error> {
        ...
        for (key, value) in &entries {
            ...
            edges.push(ReadTransaction::<B>::deserialize_edge(edge_id, value)?);
        }
    ...
    pub fn validate_all(&self) -> Result<Vec<ConstraintViolation>, Error> {
        use phonograph::constraint::{EdgeChange, NodeChange};
    ...
    fn run_validators(
        &self,
        node_changes: &[phonograph::constraint::NodeChange],
        edge_changes: &[phonograph::constraint::EdgeChange],
    ...
    pub fn commit(mut self) -> Result<(), Error> {
        ...
        {
            let mut current = self.inner.current_snapshot.write();
            *current = Arc::new(new_snapshot);
        }
        {
            let mut global_cache = self.inner.schema_cache.write();
    ...
    fn as_base_read_txn(&self) -> ReadTransaction<'_, B> {
        ReadTransaction::<B>::new(
            self.inner,
    ...
    fn read_base_node(&self, id: NodeId) -> Result<Option<Node>, Error> {
        ...
        match rtx.storage_search(self.snapshot.roots.node_store, &key)? {
            Some(value) => Ok(Some(ReadTransaction::<B>::deserialize_node(id, &value)?)),
            None => Ok(None),
        }
    }
    ...
    fn read_base_edge(&self, id: EdgeId) -> Result<Option<Edge>, Error> {
        ...
        match rtx.storage_search(self.snapshot.roots.edge_store, &key)? {
            Some(value) => Ok(Some(ReadTransaction::<B>::deserialize_edge(id, &value)?)),
            None => Ok(None),
        }
    }
}

impl<B: crate::backend::StorageBackend> Drop for WriteTransaction<'_, B> {
    fn drop(&mut self) {
        if !self.finished {
...
struct BaseSnapshotReader<'a, 'db, B: crate::backend::StorageBackend> {
    txn: &'a WriteTransaction<'db, B>,
}

impl<'a, 'db, B: crate::backend::StorageBackend> SnapshotReader for BaseSnapshotReader<'a, 'db, B> {
    ...
    fn all_edges(&self) -> Vec<Edge> {
        ...
        for (key, value) in &entries {
            let edge_id = serialization::decode_edge_key(key);
            if let Ok(edge) = ReadTransaction::<B>::deserialize_edge(edge_id, value) {
                edges.push(edge);
            }
        }
        edges
