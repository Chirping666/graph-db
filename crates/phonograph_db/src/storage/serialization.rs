...
use phonograph::inference::ProvenanceRecord;
...
pub fn deserialize_value(data: &[u8]) -> Result<(Value, usize), StorageError> {
    ...
    match tag {
        ...
        TAG_LANG_STRING => {
            ...
            let value = core::str::from_utf8(&rest[4..4 + val_len])
                .map_err(|e| StorageError {
            ...
            let lang = core::str::from_utf8(&rest[after_val + 4..after_val + 4 + lang_len])
                .map_err(|e| StorageError {
...
pub fn deserialize_property_key_name(data: &[u8]) -> Result<String, StorageError> {
    ...
    core::str::from_utf8(&data[4..4 + len])
        .map(|s| s.to_string())
        .map_err(|e| StorageError {
...
pub fn deserialize_provenance(data: &[u8]) -> Result<ProvenanceRecord, StorageError> {
    ...
    let rule_name = core::str::from_utf8(&data[10..10 + name_len])
        .map_err(|e| StorageError {
