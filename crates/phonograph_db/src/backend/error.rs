...
//! the [`BackendError`] trait that all backend error types implement,
//! and [`BackendErrorType`] for associating an error type with a backend.
...
///
/// ```
/// use phonograph_db::backend::StorageErrorKind;
///
...
/// providing a [`kind`](BackendError::kind) method for categorized error
/// handling.
///
/// Named `BackendError` (rather than `StorageError`) to avoid collision
/// with the [`error::StorageError`](crate::error::StorageError) struct.
///
/// This trait is object-safe: `dyn BackendError` is a valid trait object.
pub trait BackendError: fmt::Debug + fmt::Display {
...
mod tests {
    ...
    // Object-safety assertion for BackendError trait
    fn _assert_backend_error_object_safe(_: &dyn BackendError) {}
}
